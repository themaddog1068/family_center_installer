...
class BankNHPavilionParser:
    def __init__(self) -> None:
        # Tracking file for storing previous events
        self.tracking_file = Path("data/tracking/bank_nh_pavilion_events_tracking.json")
    ...
    def _parse_date(self, date_str: str) -> datetime:
        ...
        try:
            ...
            # Try various date formats for Bank NH Pavilion
            formats = [
                "%b %d",  # JUL 19
                "%B %d",  # JULY 19
                "%m/%d/%Y",
    ...
    def _is_recent_event(self, event: EventInfo) -> bool:
        """Check if event is from the last 14 days (expanded window for newly announced events)."""
        try:
            event_date = self._parse_date(event.date)
            fourteen_days_ago = datetime.now() - timedelta(days=14)
            return event_date >= fourteen_days_ago
        except Exception:
            return False

    def _is_next_two_weeks(self, event: EventInfo) -> bool:
        """Check if event is within the next 2 weeks."""
        try:
            event_date = self._parse_date(event.date)
            two_weeks_from_now = datetime.now() + timedelta(days=14)
            return event_date <= two_weeks_from_now
        except Exception:
            return False
    ...
    def parse_content(self, text: str) -> dict[str, list[EventInfo]]:
        """Parse Bank NH Pavilion content to extract events."""
    ...
    def create_formatted_content(self, parsed_data: dict[str, list[EventInfo]]) -> dict:
        ...
        # 2. Events in the next 2 weeks
        # 3. Recent future events (last 14 days)
        ...
        # Then add events in the next 2 weeks
        next_two_weeks_events = [
            event for event in future_events if self._is_next_two_weeks(event)
        ]
        for event in next_two_weeks_events:
            if event not in events_to_show:
                events_to_show.append(event)

        # Finally, add recent future events if we don't have enough
        recent_future_events = [
            event for event in future_events if self._is_recent_event(event)
        ]
        for event in recent_future_events:
            if event not in events_to_show and len(events_to_show) < 8:
                events_to_show.append(event)

        # Limit to 8 events total (more for Pavilion)
        events_to_show = events_to_show[:8]
...
def parse_bank_nh_pavilion_content(text: str) -> dict:
    """Parse Bank NH Pavilion content and return formatted data."""
